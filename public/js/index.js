...
(function() {
  ...
  var handleUserSignedIn_ = function(user) {
    ...
    Utils.enableButtonAndAddClickListener(
        pageUi_.doStuffButton, handleDoStuffButtonClicked_);

    var dataRef = firebase.database().ref("userData/" + userId_);
    dataRef.once("value").then(function(snapshot) {
      var userData = snapshot.val();
      if (userData) {
        userData_ = userData;
        console.log("Retrieved data: " + userData["y"]);
      }
    }).catch(function() {
...
var handleDoStuffButtonClicked_ = function(event) {
    event.preventDefault();
    console.log("Saving data to Firebase: " + userId_);
    var dataRef = firebase.database().ref("userData/" + userId_);
        dataRef.set({"x" : 5, "y": 10});
}
  ...
  var init_ = function() {
    ...
    pageUi_.doStuffButton = document.getElementById("do-things");
